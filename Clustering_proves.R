...
plot(adm_type_id,n_med,col=c1,main="Clustering of credit data in 3 classes")
...
names(diabetic_data)
#disch_id
boxplot(diabetic_data[,9]~c2, horizontal=TRUE)
#adm_source_id
boxplot(diabetic_data[,10]~c2, horizontal=TRUE)
#time_in_hpt
boxplot(diabetic_data[,11]~c2, horizontal=TRUE)

pairs(dcon[,1:7], col=c2)

plot(disch_id,adm_source_id,col=c2,main="Clustering of credit data in 3 classes")
#legend("topright",levels(c2),pch=1,col=c(1:4), cex=0.6)

cdg <- aggregate(as.data.frame(dcon),list(c2),mean)
cdg

plot(n_lab_proc, n_med, col= c2)
